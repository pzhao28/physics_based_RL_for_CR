...
class env():
    ...
    def step(self, action):
        ...
        if self.numberLoop > self.N:
            ...
            self.N = np.random.choice(np.append(np.arange(1,10), 100))
        ...
        for aircraft in self.aircrafts:
            ...
            if sqrt((aircraft.x - own_x)**2 + (aircraft.y - own_y)**2) < 8*scale:
                self.reward -= 1
    ...
    def drawDestination(self):
        dest = intruder(radius=20*scale, color=(0,255,0))
        dest.x = self.det_x
        dest.y = self.det_y
        dest.shade()
        dest.draw(win_dest)
    ...
    def act_reward(self):
        ...
        return (1-np.exp(2*np.abs(self.theta - alpha)))*0.0001
