...
class test:
    ...
    def run(self, policy):
        ...
        for i in range(testConf.num):
            for t in range(testConf.steps[i]):
                ...
                self.drawDestination(Env.win_dest)
                self.state = pygame.surfarray.array3d(Env.win)
                self.dest = pygame.surfarray.array3d(Env.win_dest)
                action = self.act(policy, test_memory)
    ...
    def act(self, policy, test_memory):
        ...
        return policy.select_action(state_input, test_memory)

    def drawDestination(self, window):
        dest = Env.intruder(radius=20 * Env.scale, color=(0, 255, 0))
        dest.x = self.det_x
        dest.y = self.det_y
        dest.shade()
        dest.draw(window)
...
def main():
    ...
    for i_episode in range(1, max_episodes + 1):
        envs.reset()
        frame, frame_dest, reward, _ = envs.step([2]*agent_num)
