...
class ActorCritic(nn.Module):
    ...
    def evaluate(self, states, distance, actions):
        #states = torch.stack(states)
        states_input = states.view(-1,*states.shape[-3:])

        actor_critic_input = torch.cat((self.conv(states_input).view(-1, self.size), distance.view(-1,1)),1).to(device)
...
class PPO:
    ...
    def select_action(self, state, distance, memory):
        #state = torch.FloatTensor(state.reshape(1,-1)).to(device)
        return self.policy_old.act(state, distance, memory).cpu().data.numpy().flatten()
    ...
    def update(self, memory):
        ...
        old_distance = torch.squeeze(torch.stack(memory.distance).to(device), 1).detach()
        old_actions = torch.squeeze(torch.stack(memory.actions).to(device), 1).detach()
        old_logprobs = torch.squeeze(torch.stack(memory.logprobs), 1).detach()
        for _ in range(self.K_epochs):
            # Evaluating old actions and values
            logprobs, state_values, dist_entropy = self.policy.evaluate(old_states, old_distance, old_actions)
...
def dist_dest(envs):
    dist = torch.tensor([envs.ps[0].dist_dest])
    for i in np.arange(envs.num_agents-1):
        dist = torch.cat((dist, torch.tensor([envs.ps[i+1].dist_dest])))
    return dist.view(envs.num_agents, 1).to(device)

def goal_angle(envs):
    goal = torch.tensor([envs.ps[0].goal_angle])
    for i in np.arange(envs.num_agents-1):
        goal = torch.cat((goal, torch.tensor([envs.ps[i+1].goal_angle])))
    return goal.view(envs.num_agents, 1).to(device)
...
def main():
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        episode_count = 0
        envs.reset()
        frame, reward,_, _ = envs.step([2]*agent_num)
        dist = dist_dest(envs)/1000
        goal = goal_angle(envs)/np.pi
        state = preprocess_single(frame)
        for t in range(max_timesteps):
            time_step += 1
            # Running policy_old
            action = ppo.select_action(state, goal, memory)
            frame, reward, count, done = envs.step(action)
            dist = dist_dest(envs)
            goal = goal_angle(envs)
            state = preprocess_single(frame)
        ...
        record_count.append(np.mean(episode_count))
        if i_episode % 500 == 0:
    ...
    plt.plot(np.array(record_count))
    plt.xlabel('episode')
