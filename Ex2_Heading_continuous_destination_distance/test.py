...
class test:
        ...
        for i in range(testConf.num):
            for t in range(testConf.steps[i]):
                ...
                for intruder in self.intruders:
                    ...
                    if intruder.x > -40 + max_x + intruder.radius or intruder.x < -40 + 0 - intruder.radius or intruder.y > -40 + max_y + intruder.radius or intruder.y < -40 - intruder.radius:
                ...
                self.goal_angle = -np.arccos((self.det_x - max_x/2) / self.distance)
                self.goal_angle = torch.tensor(self.goal_angle).view(1,1).to(device)
    ...
    def act(self, policy, test_memory):
        ...
        action, action_mean = policy.act(state_input, self.goal_angle/np.pi, test_memory)
...
if __name__ =="__main__":
    ...
    ppo.load_state_dict(torch.load('./PPO_continuous_solved_1.pth'))
