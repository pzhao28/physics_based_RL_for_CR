...
class envs():
    ...
    def step(self, actions):
        ...
        for i in np.arange(self.num_agents):
            ...
            self.count.append(env_info.count)
            self.done.append(env_info.local_done)
        return np.asarray(self.fr), np.asarray(self.re), np.asarray(self.count), np.asarray(self.done)
...
class env():
    def __init__(self):
        ...
        self.own_y = max_y / 2
        self.det_x = 40
        self.det_y = -1000 #destnation = (40,5)
        self.dist_dest = 1000
        self.goal_angle = 0
    ...
    def step(self, action):
        ...
        if self.numberLoop > self.N:
            ...
            #self.N = np.random.choice([1,2,3,4,5])
            #self.N = rd.randint(1,5)
            #self.N = 10
        ...
        self.dist_dest = np.sqrt((self.det_x-own_x)**2 + (self.det_y-own_y)**2)
        self.goal_angle = -np.arccos((self.det_x-own_x)/self.dist_dest)
        deviation = self.goal_angle - self.theta
        #reward += -self.dist_dest*0.00001
        reward += np.exp(-np.abs(deviation*100))*0.1
        ...
        if self.state == []:
            self.state = self.next_state
        return env_info(self.state, self.reward,self.count, self.done, self.next_state)

    def move(self, action): # action: changing of heading angle
        theta_old = self.theta
        self.theta = (action-1) * np.pi/2
        self.reward += -0.001*np.abs(theta_old-self.theta)
...
class env_info(envs):
    def __init__(self, state, reward, count, done, next_state):
