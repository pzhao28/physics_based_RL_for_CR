...
def play(env, policy, time=2000, preprocess=None):
    env.reset()

    # we need some initial input to start the loop
    frame1, reward1, is_done, _ = env.step(0)
    ...
    for _ in range(time):
        ...
        if preprocess is None:
            ...
            anim_frames.append(preprocess(frame2))
...
#        envs.step(np.random.choice([RIGHT, LEFT],n))
    
    # fr1 has shape (n, 210, 160, 3)
    # re1 is a simple vector of all the rewards
    fr1, re1, _, _ = envs.step([0]*n)
...
def surrogate(policy, old_probs, states, actions, rewards,
    ...
    entropy = new_probs*torch.log(new_probs+1.e-10)+ \
        (1.0-new_probs)*torch.log(1.0-new_probs+1.e-10)
...
class Policy(nn.Module):
    ...
    def __init__(self):
        ...
        # 39x39 to 18x18
        self.conv2 = nn.Conv2d(4, 32, kernel_size=5, stride=2, bias=False)
        self.size=32*18*18
