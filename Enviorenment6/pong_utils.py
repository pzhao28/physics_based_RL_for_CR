...
def play(env, policy, time=2000, preprocess=None, nrand=5):
    ...
    for _ in range(nrand):
        env_information1 = env.step(np.random.choice([RIGHT,LEFT]))
        frame1 = env_information1.next_state
        env_information2 = env.step(0)
...
def collect_trajectories(envs, policy, tmax=200, nrand=5, eps=0):
    ...
    for _ in range(nrand):
        fr1, re1, _ = envs.step(np.random.choice([RIGHT, LEFT],n))
    ...
    for t in range(tmax):
        ...
        if rand.random() > eps:
            actions = np.argmax(probs.data, axis=1)
        else:
            actions = np.array([rand.choice(np.arange(action_size)) for _ in range(envs.num_agents)])
        probs = probs[np.arange(probs.shape[0]), actions]
...
def states_to_prob(policy, states, actions):
    ...
    actions = actions.cpu().numpy().reshape(1,-1)
    probs = policy(policy_input)
    return probs[np.arange(probs.shape[0]), actions].view(states.shape[:-3])
