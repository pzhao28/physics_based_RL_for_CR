...
for e in range(episode):
    ...
    old_probs, states, actions, rewards, conf_num = \
        pong_utils.collect_trajectories(envs, envs_dest, policy, tmax=tmax)

    total_rewards = np.sum(rewards, axis=0)
    total_conf_num = np.sum(conf_num, axis=0)
    ...
    mean_conf_num.append(np.mean(total_conf_num))

    # display some progress every 20 iterations
    if (e + 1) % 20 == 0:
        print("Episode: {0:d}, mean score: {1:f}, conflict number: {2:f}".format(e + 1, np.mean(mean_rewards), np.mean(mean_conf_num)))
        #print(total_rewards)
