...
for e in range(episode):
    ...
    if e == 1:
        conflict_num_ = []
        for num_intruders in np.arange(1, 20, 1):
            testEnvs = EnvTest.envs(num_intruders=num_intruders)
            old_probs, states, actions, rewards = \
                pong_utils.collect_trajectories(testEnvs, policy, tmax=5000)

            total_rewards = np.sum(rewards, axis=0)
            conflict_num_.append(-np.mean(total_rewards))
    ...
    if e == 499:
        ...
        for num_intruders in np.arange(1, 20, 1):
            ...
            total_rewards = np.sum(rewards, axis=0)
            conflict_num.append(-np.mean(total_rewards))
        x = np.arange(1,20,1)
        plt.plot(x+1, np.array(conflict_num_)/25, label = 'no conflict resolution')
        plt.plot(x+1, np.array(conflict_num)/25, label = 'conflict resolution')
        plt.xlabel('average number of intruders in view')
        plt.ylabel('average number of conflicts per two hours flight')
        plt.legend()
        plt.show()
        np.savetxt('conflict_num_CR.csv', np.array(conflict_num)/25)
        np.savetxt('conflict_num_noCR.csv', np.array(conflict_num_)/25)
...
np.savetxt('data_N_rand.csv', mean_rewards, newline = '\n')
plt.plot((-1)*mean_rewards)
plt.xlabel('episode')
plt.ylabel('average conflict number per two flight hours')
plt.show()
plt.savefig('discrete_action.png')
#pong_utils.play(env, policy, time=200)
torch.save(policy.state_dict(), './policy.pth')
print(max_num_ac, min_num_ac)
