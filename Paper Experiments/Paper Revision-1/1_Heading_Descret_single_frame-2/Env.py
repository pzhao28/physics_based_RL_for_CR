...
class intruder(object):
    def __init__(self, vel_range = np.arange(1.5,3,0.1)*scale, radius = 5*scale, color = (255, 0, 0)): #radius=5*scale means 5 nautical miles; vel = 8 nautical miles per minitue
...
class env():
    ...
    def step(self, action):
        ...
        if action == 0: # up (default)
            ...
            moving = [2,1.5] # right up
            #if self.det_x > 40:
            #    reward+=0.0001
        elif action == 2:
            moving = [-2,1.5] # left up
            #if self.det_x < 40:
            #    reward+=0.0001
    ...
    def draw_flight_plan(self):
        ...
        pygame.draw.line(win, (0,200,0), line_start, line_end, 1)
