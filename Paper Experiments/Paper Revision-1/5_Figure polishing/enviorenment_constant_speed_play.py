...
while run:
    ...
    if i == 1000:
        dict_ac = dict()
        for ii in range(len(aircrafts)):
            dict_ac['ac{}'.format(ii)] = aircrafts[ii]
        sio.savemat('aircrafts0.mat',dict_ac)
    if i == 2000:
        dict_ac = dict()
        for ii in range(len(aircrafts)):
            dict_ac['ac{}'.format(ii+100)] = aircrafts[ii]
        sio.savemat('aircrafts1.mat', dict_ac)
    if i == 3000:
        dict_ac = dict()
        for ii in range(len(aircrafts)):
            dict_ac['ac{}'.format(ii+200)] = aircrafts[ii]
        sio.savemat('aircrafts2.mat', dict_ac)
