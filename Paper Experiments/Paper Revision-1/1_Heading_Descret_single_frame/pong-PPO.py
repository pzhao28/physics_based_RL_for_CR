...
for e in range(episode):
    ...
    if e == 1:
        conflict_num_ = []
        for num_intruders in np.arange(1, 21, 1):
            testEnvs = EnvTest.envs(num_intruders=num_intruders)
            old_probs, states, actions, rewards = \
                pong_utils.collect_trajectories(testEnvs, policy, tmax=tmax)
    ...
    if e == 699:
        ...
        x = np.arange(1,21,1)
        plt.plot(x+1, conflict_num_, label = 'no conflict resolution')
        plt.plot(x+1, conflict_num, label = 'conflict resolution')
...
np.savetxt('data.csv', mean_rewards, newline = '\n')
