...
class PPO:
    ...
    def select_action(self, state, memory):
        ...
        #return self.policy_old.act(state, memory).cpu().data.numpy()
...
def main(loop):
    ...
    for i_episode in range(1, max_episodes + 1):
        episode_reward = 0
        envs.reset()
        actions_init = 2*torch.ones(agent_num,2)
        frame, reward, _ = envs.step(actions_init)
        ...
        if i_episode % log_interval == 0:
            ...
            running_reward = 0
            avg_length = 0
    np.savetxt('data_combine_{}.csv'.format(loop), record_reward_all, newline='\n')
