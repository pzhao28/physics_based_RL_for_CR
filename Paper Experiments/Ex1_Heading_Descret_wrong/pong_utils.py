...
def collect_trajectories(envs, policy, tmax=200, nrand=5):
    ...
    for _ in range(nrand):
        fr1, re1, _, _ = envs.step(np.random.choice([UP, RIGHT, LEFT],n))
        fr2, re2, _, _ = envs.step([3]*n)
    ...
    for t in range(tmax):
        ...
        fr1, re1, is_done, conf_num = envs.step(actions)
        fr2, re2, is_done, _ = envs.step([3]*n)
        reward = re1 + re2
        # store the result
    ...
    return prob_list, state_list, \
        action_list, reward_list, conf_num_list
