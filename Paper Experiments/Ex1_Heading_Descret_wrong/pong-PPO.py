...
for e in range(episode):
    ...
    old_probs, states, actions, rewards, conf_nums = \
        pong_utils.collect_trajectories(envs, policy, tmax=tmax)

    total_rewards = np.sum(rewards, axis=0)
    total_conf = np.sum(conf_nums, axis=0)
    ...
    if (e + 1) % 20 == 0:
        print("Episode: {0:d}, score: {1:f}, conflicts: {2:f}".format(e + 1, np.mean(total_rewards), np.mean(total_conf)))
        #print(total_rewards)
