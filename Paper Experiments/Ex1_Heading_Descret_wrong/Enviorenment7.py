...
class envs():
    def __init__(self, num_agents=4, destination=(0, -1000)):

        self.ps = []
        self.num_agents = num_agents
        self.dest = destination
        for i in np.arange(num_agents):
            self.ps.append(env(self.dest))

    def step(self, actions):
        self.fr = []
        self.re = []
        self.done = []
        self.conflict_number = []
        ...
        for i in np.arange(self.num_agents):
            ...
            self.conflict_number.append(env_info.conflict_number)
        return np.asarray(self.fr), np.asarray(self.re), np.asarray(self.done), np.asarray(self.conflict_number)
...
class env():
    def __init__(self, destination):
        ...
        self.color = (255, 0, 0)
        self.dest = destination
        self.det_x = destination[0]
        self.det_y = destination[1]
        self.conflict_number = 0

    def reset(self):
        self.__init__(self.dest)
    ...
    def step(self, action):
        ...
        for aircraft in self.aircrafts:
            ...
            dist_dest = np.sqrt((aircraft.x - self.det_x)**2 + (aircraft.y - self.det_y)**2)
            theta = -np.arccos((self.det_x - aircraft.x)/dist_dest)
            act_cost = (1 - np.exp(2*np.abs(theta - alpha)))*0.01
            reward += act_cost
        ...
        if self.state == []:
            self.state = self.next_state
        return env_info(self.state, self.reward, self.done, self.next_state, self.conflict_number)

    def move(self, action): # action: changing of heading angle
        self.theta += action
        moving = (self.own_vel*np.cos(self.theta), self.own_vel*np.sin(self.theta))
        return moving
...
class env_info(envs):
    def __init__(self, state, reward, done, next_state, conflict_number):
